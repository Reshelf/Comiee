...
class AdultController extends Controller
{
    ...
    */
    public function __invoke(Request $request)
    {
        ...
        if ($sort != null) {
            if ($sort === '閲覧回数') {
                \Cache::remember("ranking.adult.views", Carbon::now()->addHour(), function () use ($query) {
                    return $query->where('views', '>', 0)->orderBy('views', 'desc')->get();
                });
            }
            if ($sort === 'お気に入り数') {
                \Cache::remember("ranking.adult.likes", Carbon::now()->addHour(), function () use ($query) {
        ...
        if ($feature != null) {
            if ($feature === '完結作品のみ') {
                \Cache::remember("ranking.adult.is_complete", Carbon::now()->addHour(), function () use ($query) {
