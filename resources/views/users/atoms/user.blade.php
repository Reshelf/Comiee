    ...
    <div class="relative z-auto">
        @empty($user->thumbnail)
            <img src="/img/bg.svg" alt=""
                class="dark:hidden h-[150px] lg:h-[300px] rounded-b flex w-full object-cover">
    ...
    <div
        ...
        <div class="w-full md:px-6 flex justify-between md:mt-4">
            <div class="w-full flex flex-col">
                ...
                <div class="text-left lg:text-lg text-t-color-3">
                ...
                <div class="flex justify-start items-center text-sm pt-2 md:pt-2">
                    <a href="{{ route('users.followings', ['lang' => app()->getLocale(), 'username' => $user->username]) }}"
                        ...
                        <span class="text-t-color-3 pl-1 text-xs lg:text-base">{{ __('フォロー') }}</span>
                    </a>
                    <a href="{{ route('users.followers', ['lang' => app()->getLocale(), 'username' => $user->username]) }}"
                        ...
                        <span class="text-t-color-3 pl-1 text-xs lg:text-base">{{ __('フォロワー') }}</span>
                    </a>
                    @if (Auth::id() !== $user->id)
            ...
            <div class="relative flex items-center">
                @if ($mypage)
                    <a href="{{ route('users.show', ['lang' => app()->getLocale(), 'username' => $user->username]) }}"
                        class="{{ $mypage ? 'border-primary text-primary dark:text-ddd font-bold' : 'border-transparent hover:text-primary dark:hover:text-light hover:font-semibold  dark:border-dark' }} py-3 px-6 border-b-2 text-xs lg:text-[14px]">{{ __('投稿作品') }}</a>
                    @if (Auth::id() === $user->id)
                        <a href="{{ route('users.settings', ['lang' => app()->getLocale(), 'username' => $user->username]) }}"
                            class="{{ $settings ? 'border-primary text-primary dark:text-ddd font-bold' : 'border-transparent hover:text-primary dark:hover:text-light hover:font-semibold  dark:border-dark' }} py-3 px-6 border-b-2 text-xs lg:text-[14px]">{{ __('設定') }}</a>
                    @endif
                @else
                    @if (Auth::id() === $user->id)
