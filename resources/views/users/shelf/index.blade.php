        ...
        <div class="setting-tab">
          <ul class="tabMenu scroll-none sticky top-0 lg:h-[300px]">
            ...
            <a href="{{ route('user.shelf.like', ['lang' => app()->getLocale()]) }}"
              class="{{ $type === 'like' ? 'font-bold text-primary dark:text-ddd' : '' }} ">{{ __('お気に入り') }}</a>
            <a href="{{ route('user.shelf.view', ['lang' => app()->getLocale()]) }}"
              class="{{ $type === 'view_history' ? 'font-bold text-primary dark:text-ddd' : '' }} ">{{ __('閲覧履歴') }}</a>
            <a href="{{ route('user.shelf.purchase', ['lang' => app()->getLocale()]) }}"
              class="{{ $type === 'purchase_history' ? 'font-bold text-primary dark:text-ddd' : '' }} ">{{ __('購入履歴') }}</a>
          </ul>
