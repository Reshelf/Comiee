        ...
        <form method="POST" action="{{ route('password.update', app()->getLocale()) }}" class="px-6 dark:bg-dark-1 pt-6">
            ...
            <div class="w-full mb-3">
                <input
                    ...
                    type="password" name="password" required placeholder="{{ __('新しいパスワード') }}">
            </div>
            <div class="w-full mb-3">
                <input
                    class="w-full text-[15px] px-2 py-3 rounded-[3px] border border-l-0 border-r-0 border-t-0 border-b-ccc dark:border-b-dark dark:bg-dark-2"
                    type="password" name="password_confirmation" required placeholder="{{ __('パスワード') }}">
            </div>
            <input type="hidden" name="remember" value="on">
            <button type="submit" class="btn-primary px-6 py-4 w-full mb-4">{{ __('送信') }}</button>
        </form>
    </div>
@endsection
