<div class="overflow-x-scroll inline-flex items-center mb-4 scroll-none">
    @if ($ranking)
        ...
        <a href="{{ route('ranking.adult', app()->getLocale()) }}"
            class="{{ $genre === 5 ? 'border-[#b824e1] text-[#b824e1] dark:text-ddd font-bold' : 'border-transparent hover:text-[#b824e1] dark:hover:text-ddd hover:font-semibold dark:border-dark' }} text-sm md:text-base py-3 px-6 border-b-2 whitespace-nowrap">{{ __('オトナ') }}</a>
    @endif
    @if ($todays_new)
